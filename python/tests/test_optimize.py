...
import pytest
from datem.optimizer import optimize_ibis, optimize_sql
...
def test_roundtrip_group(con, g):
    ...
    expected = con.execute(original).sort_values(by="a").reset_index(drop=True)
...
def test_roundtrip_distinct(con, g):
    ...
    expected = (
        con.execute(original).sort_values(by=["a", "f", "g"]).reset_index(drop=True)
    )
